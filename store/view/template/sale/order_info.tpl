...
<div class="breadcrumbox">
  ...
  <div class="page-header">
    <div class="container-fluid">
      <div class="pull-right"><a href="<?php echo $invoice; ?>" target="_blank" data-toggle="tooltip" title="<?php echo $button_invoice_print; ?>" class="btn btn-info"><i class="fa fa-print"></i></a> <a href="<?php echo $shipping; ?>" target="_blank" data-toggle="tooltip" title="<?php echo $button_shipping_print; ?>" class="btn btn-info"><i class="fa fa-truck"></i></a>
<?php /*
...
*/ ?>
              ...
              <tr>
                ...
                <td id="show_product_total" ><?php //echo $total; ?></td>
              </tr>
		...
		foreach ($product_wise_shipping as $product_shipping) {  
		...
		foreach ($totals as $totall) { 
	        ...
	        foreach ($totals as $totals) { ?>
			 ...
			 <?php }elseif( $totals['title'] == 'Total' ) {
					...
					foreach($products as $products_1){ if(isset($taxes_product)){
					foreach ($taxes_product as $taxes_product_1) {					
...
$('#button-history').on('click', function() {
  if(typeof verifyStatusChange == 'function'){
	...
	$.ajax({
		url: 'index.php?route=sale/order/api&token=<?php echo $token; ?>&api=api/order/history&flag=0&order_id=<?php echo $order_id; ?>',
		type: 'post',
		dataType: 'json',
		data: 'order_status_id=' + encodeURIComponent($('select[name=\'order_status_id\']').val()) + '&notify=' + ($('input[name=\'notify\']').prop('checked') ? 1 : 0) + '&append=' + ($('input[name=\'append\']').prop('checked') ? 1 : 0) + '&comment=' + encodeURIComponent($('textarea[name=\'comment\']').val()) + '&sdID=' + encodeURIComponent($('input[name=\'sdID\']').val()),
		beforeSend: function() {
...
$(document).ready(function() {
  //changeStatus(); 
	$('#show_product_total').html( $('#get_total_product').text() ); 
});
