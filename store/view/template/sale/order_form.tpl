  ...
  <div class="container-fluid">
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        <form class="form-horizontal"><input type="hidden" value="<?php echo $sdID; ?>" name="MI" />
		...
		<input type="hidden" name="od_status" value="1" id="input-sts" class="form-control">
                  </div>
                </div>
                <div class="form-group" style="display:none;">
...
var order_id = $('input[name=\'order_id\']').val();
	$.ajax({
		url: 'index.php?route=sale/order/api&token=<?php echo $token; ?>&api=api/cart/products&store_id=' + $('select[name=\'store_id\'] option:selected').val() + '&order_id=' + order_id ,
		dataType: 'json',
...
$('#button-save').on('click', function() {
	var order_id = $('input[name=\'order_id\']').val();
	var vendor_id = $('input[name=\'MI\']').val();
	
	if (order_id == 0) {
		var url = 'index.php?route=sale/order/api&token=<?php echo $token; ?>&api=api/order/add&store_id=' + $('select[name=\'store_id\'] option:selected').val() + '&vendor_id=' + vendor_id;
	} else {
		var url = 'index.php?route=sale/order/api&token=<?php echo $token; ?>&api=api/order/edit&store_id=' + $('select[name=\'store_id\'] option:selected').val() + '&order_id=' + order_id + '&vendor_id=' + vendor_id;
	}
	
	$.ajax({
		url: url,
		type: 'post',
		data: $('#tab-total select[name=\'order_status_id\'], #tab-total select, #tab-total textarea[name=\'comment\'], #tab-total input[name=\'affiliate_id\'], #tab-total input[name=\'od_status\']'),
		dataType: 'json',
		beforeSend: function() {
