...
class ControllerLocalisationCountry extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('localisation/country', 'token=' . $this->session->data['mtoken'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('localisation/country', 'token=' . $this->session->data['mtoken'] . $url, 'SSL')
		);

		$data['add'] = $this->url->link('localisation/country/add', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
		$data['delete'] = $this->url->link('localisation/country/delete', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			$data['countries'][] = array(
				...
				'edit'       => $this->url->link('localisation/country/edit', 'token=' . $this->session->data['mtoken'] . '&country_id=' . $result['country_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_name'] = $this->url->link('localisation/country', 'token=' . $this->session->data['mtoken'] . '&sort=name' . $url, 'SSL');
		$data['sort_iso_code_2'] = $this->url->link('localisation/country', 'token=' . $this->session->data['mtoken'] . '&sort=iso_code_2' . $url, 'SSL');
		$data['sort_iso_code_3'] = $this->url->link('localisation/country', 'token=' . $this->session->data['mtoken'] . '&sort=iso_code_3' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('localisation/country', 'token=' . $this->session->data['mtoken'] . $url . '&page={page}', 'SSL');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['country_id'])) {
			$data['action'] = $this->url->link('localisation/country/add', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('localisation/country/edit', 'token=' . $this->session->data['mtoken'] . '&country_id=' . $this->request->get['country_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('localisation/country', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
	...
	protected function validateForm() {
		if (!$this->merchant->hasPermission('modify', 'localisation/country')) {
