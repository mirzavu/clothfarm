...
class ControllerMarketingContact extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('marketing/contact', 'token=' . $this->session->data['mtoken'], 'SSL')
		);

		$data['cancel'] = $this->url->link('marketing/contact', 'token=' . $this->session->data['mtoken'], 'SSL');
	...
	public function send() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			if (!$this->merchant->hasPermission('modify', 'marketing/contact')) {
			...
			if (!$json) {
				...
				if ($emails) {
					...
					if ($end < $email_total) {
						$json['next'] = str_replace('&amp;', '&', $this->url->link('marketing/contact/send', 'token=' . $this->session->data['mtoken'] . '&page=' . ($page + 1), 'SSL'));
					} else {
						$json['next'] = '';
					}
