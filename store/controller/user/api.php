...
class ControllerUserApi extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('user/api', 'token=' . $this->session->data['mtoken'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('user/api', 'token=' . $this->session->data['mtoken'] . $url, 'SSL')
		);

		$data['add'] = $this->url->link('user/api/add', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
		$data['delete'] = $this->url->link('user/api/delete', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			$data['apis'][] = array(
				...
				'edit'       => $this->url->link('user/api/edit', 'token=' . $this->session->data['mtoken'] . '&api_id=' . $result['api_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_username'] = $this->url->link('user/api', 'token=' . $this->session->data['mtoken'] . '&sort=username' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('user/api', 'token=' . $this->session->data['mtoken'] . '&sort=status' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('user/api', 'token=' . $this->session->data['mtoken'] . '&sort=date_added' . $url, 'SSL');
		$data['sort_date_modified'] = $this->url->link('user/api', 'token=' . $this->session->data['mtoken'] . '&sort=date_modified' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('user/api', 'token=' . $this->session->data['mtoken'] . $url . '&page={page}', 'SSL');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['api_id'])) {
			$data['action'] = $this->url->link('user/api/add', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('user/api/edit', 'token=' . $this->session->data['mtoken'] . '&api_id=' . $this->request->get['api_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('user/api', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
	...
	protected function validateForm() {
		if (!$this->merchant->hasPermission('modify', 'user/user')) {
	...
	protected function validateDelete() {
		if (!$this->merchant->hasPermission('modify', 'user/api')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
