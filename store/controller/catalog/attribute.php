...
class ControllerCatalogAttribute extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('catalog/attribute', 'token=' . $this->session->data['mtoken'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/attribute', 'token=' . $this->session->data['mtoken'] . $url, 'SSL')
		);

		$data['add'] = $this->url->link('catalog/attribute/add', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
		$data['delete'] = $this->url->link('catalog/attribute/delete', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			$data['attributes'][] = array(
				...
				'edit'            => $this->url->link('catalog/attribute/edit', 'token=' . $this->session->data['mtoken'] . '&attribute_id=' . $result['attribute_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_name'] = $this->url->link('catalog/attribute', 'token=' . $this->session->data['mtoken'] . '&sort=ad.name' . $url, 'SSL');
		$data['sort_attribute_group'] = $this->url->link('catalog/attribute', 'token=' . $this->session->data['mtoken'] . '&sort=attribute_group' . $url, 'SSL');
		$data['sort_sort_order'] = $this->url->link('catalog/attribute', 'token=' . $this->session->data['mtoken'] . '&sort=a.sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('catalog/attribute', 'token=' . $this->session->data['mtoken'] . $url . '&page={page}', 'SSL');
	...
	protected function getForm() {
		...
		if (isset($this->error['attribute_group_id'])) { 
			$data['error_attr_gr_store'] = $this->error['attribute_group_id'];
		} else {
			$data['error_attr_gr_store'] = '';
		}
		...
		if (!isset($this->request->get['attribute_id'])) {
			$data['action'] = $this->url->link('catalog/attribute/add', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('catalog/attribute/edit', 'token=' . $this->session->data['mtoken'] . '&attribute_id=' . $this->request->get['attribute_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('catalog/attribute', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
	...
	protected function validateForm() {
		if (!$this->merchant->hasPermission('modify', 'catalog/attribute')) {
		...
		if(isset($this->request->post['attribute_group_id'])){
		}else{	$this->error['attribute_group_id'] = $this->language->get('error_attr_gr_store');
		}	
