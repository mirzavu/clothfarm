...
class ControllerCatalogBankDetails extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('catalog/bank_details', 'token=' . $this->session->data['mtoken'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/bank_details', 'token=' . $this->session->data['mtoken'] . $url, 'SSL')
		);

		$data['add'] = $this->url->link('catalog/bank_details/add', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
		$data['delete'] = $this->url->link('catalog/bank_details/delete', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			$banStores = $this->model_catalog_bank_details->getBank_detailsStores($result['bank_details_id']);
			if(count($banStores) == 1 && $banStores[0] != 0 && $banStores[0] == $this->session->data['storeID'] ){ $CE = 1; }else{ $CE = 2; } 
			$data['bank_detailss'][] = array(
				...
				'can_edit_cat'=> $CE,
				'edit'            => $this->url->link('catalog/bank_details/edit', 'token=' . $this->session->data['mtoken'] . '&bank_details_id=' . $result['bank_details_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_name'] = $this->url->link('catalog/bank_details', 'token=' . $this->session->data['mtoken'] . '&sort=name' . $url, 'SSL');
		$data['sort_bank_name'] = $this->url->link('catalog/bank_details', 'token=' . $this->session->data['mtoken'] . '&sort=bank_name' . $url, 'SSL');
		$data['sort_account_no'] = $this->url->link('catalog/bank_details', 'token=' . $this->session->data['mtoken'] . '&sort=account_no' . $url, 'SSL');
		$data['sort_branch_name'] = $this->url->link('catalog/bank_details', 'token=' . $this->session->data['mtoken'] . '&sort=branch_name' . $url, 'SSL');
		$data['sort_ifsc_code'] = $this->url->link('catalog/bank_details', 'token=' . $this->session->data['mtoken'] . '&sort=ifsc_code' . $url, 'SSL');
		$data['sort_sort_order'] = $this->url->link('catalog/bank_details', 'token=' . $this->session->data['mtoken'] . '&sort=sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('catalog/bank_details', 'token=' . $this->session->data['mtoken'] . $url . '&page={page}', 'SSL');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['bank_details_id'])) {
			$data['action'] = $this->url->link('catalog/bank_details/add', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('catalog/bank_details/edit', 'token=' . $this->session->data['mtoken'] . '&bank_details_id=' . $this->request->get['bank_details_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('catalog/bank_details', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
	...
	protected function validateForm() {
		if (!$this->merchant->hasPermission('modify', 'catalog/bank_details')) {
