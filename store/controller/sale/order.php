...
class ControllerSaleOrder extends Controller {
	...
	public function delete() {
		...
		if (isset($this->session->data['cookie'])) {
			...
			curl_setopt($curl, CURLOPT_FORBID_REUSE, false);
			curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
			curl_setopt($curl, CURLOPT_URL, HTTPS_CATALOG . 'index.php?route=api/order/delete&order_id=' . $this->request->get['order_id'].'&vendor_id='.$vendor_id);
		...
		if (isset($response['success'])) {
			...
			$this->response->redirect($this->url->link('sale/order', 'token=' . $this->session->data['mtoken'] . $url, 'SSL'));
		}
	...
	protected function getList() { 
		...
		$data['invoice'] = $this->url->link('sale/order/invoice', 'token=' . $this->session->data['mtoken'], 'SSL');
		$data['shipping'] = $this->url->link('sale/order/shipping', 'token=' . $this->session->data['mtoken'], 'SSL');
		$data['add'] = $this->url->link('sale/order/add', 'token=' . $this->session->data['mtoken'], 'SSL');
		...
		foreach ($results as $result) {
			$no_of_products = $this->model_sale_order->getNumProducts($result['order_id']); 

			$order_info_vendor = $this->model_sale_order->getOrderVendor($result['order_id']); 
			$order_amount_vendor = $this->model_sale_order->getTotalOrderAmount($result['order_id']); 

			$order_status_info = $this->model_localisation_order_status->getOrderStatus($order_info_vendor['order_status_id_vendor']);
			...
			$data['orders'][] = array(
				...
				'total'         => $this->currency->format($order_amount_vendor, $result['currency_code'], $result['currency_value']),
				'date_added'    => date($this->language->get('date_format_short'), strtotime($result['date_added'])),
				'date_modified' => date($this->language->get('date_format_short'), strtotime($result['date_modified'])),
				'shipping_code' => $result['shipping_code'],
				'view'          => $this->url->link('sale/order/info', 'token=' . $this->session->data['mtoken'] . '&order_id=' . $result['order_id'] . $url, 'SSL'),
				'edit'          => $this->url->link('sale/order/edit', 'token=' . $this->session->data['mtoken'] . '&order_id=' . $result['order_id'] . $url, 'SSL'),
				'delete'        => $this->url->link('sale/order/delete', 'token=' . $this->session->data['mtoken'] . '&order_id=' . $result['order_id'] . $url, 'SSL')
			);
		}
		...
		$data['column_no_products'] = $this->language->get('column_no_products');
		...
		$data['sort_order'] = $this->url->link('sale/order', 'token=' . $this->session->data['mtoken'] . '&sort=o.order_id' . $url, 'SSL');
		$data['sort_customer'] = $this->url->link('sale/order', 'token=' . $this->session->data['mtoken'] . '&sort=customer' . $url, 'SSL');
		$data['sort_email'] = $this->url->link('sale/order', 'token=' . $this->session->data['mtoken'] . '&sort=email' . $url, 'SSL');
		$data['sort_phone'] = $this->url->link('sale/order', 'token=' . $this->session->data['mtoken'] . '&sort=telephone' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('sale/order', 'token=' . $this->session->data['mtoken'] . '&sort=status' . $url, 'SSL');
		$data['sort_total'] = $this->url->link('sale/order', 'token=' . $this->session->data['mtoken'] . '&sort=o.total' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('sale/order', 'token=' . $this->session->data['mtoken'] . '&sort=o.date_added' . $url, 'SSL');
		$data['sort_date_modified'] = $this->url->link('sale/order', 'token=' . $this->session->data['mtoken'] . '&sort=o.date_modified' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('sale/order', 'token=' . $this->session->data['mtoken'] . $url . '&page={page}', 'SSL');
	...
	public function getForm() {
		...
		$data['token'] = $this->session->data['mtoken'];
		$data['sdID'] = $this->session->data['storeID'];
		...
		$data['cancel'] = $this->url->link('sale/order', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');

		if (isset($this->request->get['order_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$order_info = $this->model_sale_order->getOrder($this->request->get['order_id']);
			$order_info_vendor = $this->model_sale_order->getOrderVendor($this->request->get['order_id']);
		}
		...
		if (!empty($order_info)) {
			...
			$data['order_status_id'] = $order_info_vendor['order_status_id_vendor'];
	...
	public function info() {
		...
		$order_info_vendor = $this->model_sale_order->getOrderVendor($order_id);
		$order_amount_vendor = $this->model_sale_order->getTotalOrderAmount($order_id);
		...
		if ($order_info) {
			...
			if ($order_info['customer_id']) {
				$data['customer'] = $this->url->link('sale/customer/edit', 'token=' . $this->session->data['mtoken'] . '&customer_id=' . $order_info['customer_id'], 'SSL');
			} else {
				$data['customer'] = '';
			}
			...
			$data['shipping_method'] = $order_info['shipping_method'];
			$data['payment_method'] = $order_info['payment_method'];
			$data['total'] = $this->currency->format($order_amount_vendor, $order_info['currency_code'], $order_info['currency_value']);
			...
			if ($order_info['affiliate_id']) {
				$data['affiliate'] = $this->url->link('marketing/affiliate/edit', 'token=' . $this->session->data['mtoken'] . '&affiliate_id=' . $order_info['affiliate_id'], 'SSL');
			} else {
				$data['affiliate'] = '';
			}
			...
			$data['products'] = array();
			$data['product_wise_shipping'] = array();
			...
			foreach ($products as $product) {
					...
					if ($option['type'] != 'file') {
						...
						if ($upload_info) {
							$option_data[] = array(
								...
								'value' => $upload_info['name'],
								'type'  => $option['type'],
								'href'  => $this->url->link('tool/upload/download', 'token=' . $this->session->data['mtoken'] . '&code=' . $upload_info['code'], 'SSL')
							);
						}
					}
				}
				...
				$data['products'][] = array(
					...
					'new_total'    		   => $this->currency->seperate($product['total'], $order_info['currency_code'], $order_info['currency_value']),
					'new_currency_left'        => $this->currency->getSymbolLeft($order_info['currency_code']),
					'new_currency_right'       => $this->currency->getSymbolRight($order_info['currency_code']),
					'href'     		   => $this->url->link('catalog/product/edit', 'token=' . $this->session->data['mtoken'] . '&product_id=' . $product['product_id'], 'SSL')
...
//echo '<pre>';print_r($data['product_wise_shipping']); exit;
			$data['vouchers'] = array();
			...
			foreach ($vouchers as $voucher) {
				$data['vouchers'][] = array(
					...
					'href'        => $this->url->link('sale/voucher/edit', 'token=' . $this->session->data['mtoken'] . '&voucher_id=' . $voucher['voucher_id'], 'SSL')
				);
			}
			...
			foreach ($totals as $total) {
				$data['totals'][] = array(
					...
					'new_text'  => $this->currency->seperate($total['value'], $order_info['currency_code'], $order_info['currency_value']),
					'total_currency_left' => $this->currency->getSymbolLeft($order_info['currency_code']),
					'total_currency_right'=> $this->currency->getSymbolRight($order_info['currency_code'])
				);
			}
			$tax_per_products = $this->model_sale_order->getTaxPerProduct($this->request->get['order_id']);

			foreach ($tax_per_products as $tax_per_product) {
				$data['taxes_product'][] = array(
					'product_id_1' => $tax_per_product['product_id'],
					'title_1' => $tax_per_product['title'],
					'code_1' => $tax_per_product['code'],
					'text_1'  => $this->currency->format($tax_per_product['value'], $order_info['currency_code'], $order_info['currency_value']),
					'new_text_1'  => $this->currency->seperate($tax_per_product['value'], $order_info['currency_code'], $order_info['currency_value']),					
			...
			if ($this->merchant->hasPermission('modify', 'sale/order')) {
				$this->load->model('user/api');
			...
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('error/not_found', 'token=' . $this->session->data['mtoken'], 'SSL')
			);
	...
	public function history() {
		...
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('sale/order/history', 'token=' . $this->session->data['mtoken'] . '&order_id=' . $this->request->get['order_id'] . '&page={page}', 'SSL');
	...
	public function invoice() {
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				$tax_per_products = $this->model_sale_order->getTaxPerProduct($order_id);
				...
				$data['orders'][] = array(
					...
					'product_wise_shipping' => $product_shipping_data,
					'taxes_product'       => $taxes_data
				);
			}
		}
