...
class ControllerSaleCustomerBanIp extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('sale/customer_ban_ip', 'token=' . $this->session->data['mtoken'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('sale/customer_ban_ip', 'token=' . $this->session->data['mtoken'] . $url, 'SSL')
		);

		$data['add'] = $this->url->link('sale/customer_ban_ip/add', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
		$data['delete'] = $this->url->link('sale/customer_ban_ip/delete', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			$data['customer_ban_ips'][] = array(
				...
				'ip'                 => $result['ip'],
				'total'              => $result['total'],
				'customer'           => $this->url->link('sale/customer', 'token=' . $this->session->data['mtoken'] . '&filter_ip=' . $result['ip'], 'SSL'),
				'edit'               => $this->url->link('sale/customer_ban_ip/edit', 'token=' . $this->session->data['mtoken'] . '&customer_ban_ip_id=' . $result['customer_ban_ip_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_ip'] = $this->url->link('sale/customer_ban_ip', 'token=' . $this->session->data['mtoken'] . '&sort=ip' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('sale/customer_ban_ip', 'token=' . $this->session->data['mtoken'] . $url . '&page={page}', 'SSL');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['customer_ban_ip_id'])) {
			$data['action'] = $this->url->link('sale/customer_ban_ip/add', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('sale/customer_ban_ip/edit', 'token=' . $this->session->data['mtoken'] . '&customer_ban_ip_id=' . $this->request->get['customer_ban_ip_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('sale/customer_ban_ip', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
	...
	protected function validateForm() {
		if (!$this->merchant->hasPermission('modify', 'sale/customer_ban_ip')) {
