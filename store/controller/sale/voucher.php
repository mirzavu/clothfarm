...
class ControllerSaleVoucher extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('sale/voucher', 'token=' . $this->session->data['mtoken'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('sale/voucher', 'token=' . $this->session->data['mtoken'] . $url, 'SSL')
		);

		$data['add'] = $this->url->link('sale/voucher/add', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
		$data['delete'] = $this->url->link('sale/voucher/delete', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			$data['vouchers'][] = array(
				...
				'edit'       => $this->url->link('sale/voucher/edit', 'token=' . $this->session->data['mtoken'] . '&voucher_id=' . $result['voucher_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_code'] = $this->url->link('sale/voucher', 'token=' . $this->session->data['mtoken'] . '&sort=v.code' . $url, 'SSL');
		$data['sort_from'] = $this->url->link('sale/voucher', 'token=' . $this->session->data['mtoken'] . '&sort=v.from_name' . $url, 'SSL');
		$data['sort_to'] = $this->url->link('sale/voucher', 'token=' . $this->session->data['mtoken'] . '&sort=v.to_name' . $url, 'SSL');
		$data['sort_theme'] = $this->url->link('sale/voucher', 'token=' . $this->session->data['mtoken'] . '&sort=theme' . $url, 'SSL');
		$data['sort_amount'] = $this->url->link('sale/voucher', 'token=' . $this->session->data['mtoken'] . '&sort=v.amount' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('sale/voucher', 'token=' . $this->session->data['mtoken'] . '&sort=v.date_end' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('sale/voucher', 'token=' . $this->session->data['mtoken'] . '&sort=v.date_added' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('sale/voucher', 'token=' . $this->session->data['mtoken'] . $url . '&page={page}', 'SSL');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['voucher_id'])) {
			$data['action'] = $this->url->link('sale/voucher/add', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('sale/voucher/edit', 'token=' . $this->session->data['mtoken'] . '&voucher_id=' . $this->request->get['voucher_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('sale/voucher', 'token=' . $this->session->data['mtoken'] . $url, 'SSL');
	...
	protected function validateForm() {
		if (!$this->merchant->hasPermission('modify', 'sale/voucher')) {
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $voucher_id) {
			...
			if ($order_voucher_info) {
				$this->error['warning'] = sprintf($this->language->get('error_order'), $this->url->link('sale/order/info', 'token=' . $this->session->data['mtoken'] . '&order_id=' . $order_voucher_info['order_id'], 'SSL'));
