...
class ControllerPaymentPPPayflowIframe extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('payment/pp_payflow', 'token=' . $this->session->data['mtoken'], 'SSL'),
		);

		$data['action'] = $this->url->link('payment/pp_payflow_iframe', 'token=' . $this->session->data['mtoken'], 'SSL');
	...
	public function refund() {
		...
		if ($transaction) {
			...
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('payment/pp_payflow_iframe', 'token=' . $this->session->data['mtoken'], 'SSL')
			);

			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_refund'),
				'href' => $this->url->link('payment/pp_payflow_iframe/refund', 'transaction_reference=' . $this->request->get['transaction_reference'] . '&token=' . $this->session->data['mtoken'], 'SSL')
			);

			$data['transaction_reference'] = $transaction['transaction_reference'];
			$data['transaction_amount'] = number_format($transaction['amount'], 2);
			$data['cancel'] = $this->url->link('sale/order/info', 'token=' . $this->session->data['mtoken'] . '&order_id=' . $transaction['order_id'], 'SSL');
			...
			if ($paypal_order && $order_info) {
				...
				if ($result['RESULT'] == 0) {
					...
					$actions[] = array(
						...
						'href' => $this->url->link('payment/pp_payflow_iframe/refund', 'transaction_reference=' . $result['PNREF'] . '&token=' . $this->session->data['mtoken']),
					);
				...
				switch ($transaction['transaction_type']) {
					...
					case 'S':
						...
						$actions[] = array(
							...
							'href' => $this->url->link('payment/pp_payflow_iframe/refund', 'transaction_reference=' . $transaction['transaction_reference'] . '&token=' . $this->session->data['mtoken']),
						);
						break;
					case 'D':
						$transaction_type = $this->language->get('text_capture');
	...
	protected function validate() {
		if (!$this->merchant->hasPermission('modify', 'payment/pp_payflow_iframe')) {
