...
class ControllerPaymentPPProIframe extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('extension/payment', 'token=' . $this->session->data['mtoken'], 'SSL'));
		} else {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/payment', 'token=' . $this->session->data['mtoken'], 'SSL')
		);

		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('payment/pp_pro_iframe', 'token=' . $this->session->data['mtoken'], 'SSL')
		);

		$data['action'] = $this->url->link('payment/pp_pro_iframe', 'token=' . $this->session->data['mtoken'], 'SSL');

		$data['cancel'] = $this->url->link('extension/payment', 'token=' . $this->session->data['mtoken'], 'SSL');
	...
	public function refund() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('payment/pp_pro_iframe/refund', 'token=' . $this->session->data['mtoken'], 'SSL')
		);

		//button actions
		$data['action'] = $this->url->link('payment/pp_pro_iframe/doRefund', 'token=' . $this->session->data['mtoken'], 'SSL');

		if (isset($this->request->get['order_id'])) {
			$data['cancel'] = $this->url->link('sale/order/info', 'token=' . $this->session->data['mtoken'] . '&order_id=' . $this->request->get['order_id'], 'SSL');
		} else {
			$data['cancel'] = '';
		}
	...
	public function doRefund() {
		...
		if (isset($this->request->post['transaction_id']) && isset($this->request->post['refund_full'])) {
			...
			if ($this->request->post['refund_full'] == 0 && $this->request->post['amount'] == 0) {
				...
				$this->response->redirect($this->url->link('payment/pp_pro_iframe/refund', 'token=' . $this->session->data['mtoken'] . '&transaction_id=' . $this->request->post['transaction_id'], 'SSL'));
			} else {
				...
				if ($paypal_order) {
					...
					if ($result == false) {
						...
						$this->response->redirect($this->url->link('sale/order/info', 'token=' . $this->session->data['mtoken'] . '&order_id=' . $paypal_order['order_id'], 'SSL'));
	...
	public function viewTransaction() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('payment/pp_pro_iframe/viewTransaction', 'token=' . $this->session->data['mtoken'] . '&transaction_id=' . $this->request->get['transaction_id'], 'SSL')
		);
		...
		$data['view_link'] = $this->url->link('payment/pp_pro_iframe/viewTransaction', 'token=' . $this->session->data['mtoken'], 'SSL');
		$data['token'] = $this->session->data['mtoken'];

		$this->document->setTitle($this->language->get('text_transaction'));

		if (isset($this->request->get['order_id'])) {
			$data['back'] = $this->url->link('sale/order/info', 'token=' . $this->session->data['mtoken'] . '&order_id=' . $this->request->get['order_id'], 'SSL');
		} else {
			$data['back'] = '';
		}
	...
	public function orderAction() {
		...
		if ($paypal_order) {
			...
			$data['view_link'] = $this->url->link('payment/pp_pro_iframe/viewTransaction', 'token=' . $this->session->data['mtoken'], 'SSL');
			$data['refund_link'] = $this->url->link('payment/pp_pro_iframe/refund', 'token=' . $this->session->data['mtoken'], 'SSL');
			$data['resend_link'] = $this->url->link('payment/pp_pro_iframe/resend', 'token=' . $this->session->data['mtoken'], 'SSL');
			...
			foreach ($paypal_order['transactions'] as $transaction) {
				$data['transactions'][] = array(
					...
					'view' => $this->url->link('payment/pp_pro_iframe/viewTransaction', 'token=' . $this->session->data['mtoken'] . "&transaction_id=" . $transaction['transaction_id'] . '&order_id=' . $this->request->get['order_id'], 'SSL'),
					'refund' => $this->url->link('payment/pp_pro_iframe/refund', 'token=' . $this->session->data['mtoken'] . "&transaction_id=" . $transaction['transaction_id'] . "&order_id=" . $this->request->get['order_id'], 'SSL'),
					'resend' => $this->url->link('payment/pp_pro_iframe/resend', 'token=' . $this->session->data['mtoken'] . "&paypal_iframe_order_transaction_id=" . $transaction['paypal_iframe_order_transaction_id'], 'SSL'),
				);
			}

			$data['reauthorise_link'] = $this->url->link('payment/pp_pro_iframe/reauthorise', 'token=' . $this->session->data['mtoken'], 'SSL');
	...
	protected function validate() {
		if (!$this->merchant->hasPermission('modify', 'payment/pp_pro_iframe')) {
