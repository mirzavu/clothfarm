...
class ControllerTotalKlarnaFee extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('extension/total', 'token=' . $this->session->data['mtoken'], 'SSL'));
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('common/dashboard', 'token=' . $this->session->data['mtoken'], 'SSL')
		);

		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('text_total'),
			'href' => $this->url->link('extension/total', 'token=' . $this->session->data['mtoken'], 'SSL')
		);

		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('total/klarna_fee', 'token=' . $this->session->data['mtoken'], 'SSL')
		);

		$data['action'] = $this->url->link('total/klarna_fee', 'token=' . $this->session->data['mtoken'], 'SSL');

		$data['cancel'] = $this->url->link('extension/total', 'token=' . $this->session->data['mtoken'], 'SSL');
	...
	private function validate() {
		if (!$this->merchant->hasPermission('modify', 'total/klarna_fee')) {
