...
class ModelSaleReturn extends Model {
	...
	public function getReturn($return_id) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT CONCAT(c.firstname, ' ', c.lastname) FROM " . DB_PREFIX . "customer c WHERE c.customer_id = r.customer_id) AS customer FROM `" . DB_PREFIX . "return` r WHERE r.return_id = '" . (int)$return_id . "' AND merchant_id='".$this->session->data['storeID']."'"); 

		return $query->row;
	}

	public function getReturns($data = array()) {
		$sql = "SELECT *, CONCAT(r.firstname, ' ', r.lastname) AS customer, (SELECT rs.name FROM " . DB_PREFIX . "return_status rs WHERE rs.return_status_id = r.return_status_id AND rs.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status FROM `" . DB_PREFIX . "return` r INNER JOIN `" . DB_PREFIX . "order_product` op on r.order_id = op.order_id AND r.product_id = op.product_id WHERE r.merchant_id = '". $this->session->data['storeID'] ."'";
	...
	public function getTotalReturns($data = array()) {
		$sql = "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "return`r INNER JOIN `" . DB_PREFIX . "order_product` op on r.order_id = op.order_id AND r.product_id = op.product_id WHERE r.merchant_id = '". $this->session->data['storeID'] ."'";
