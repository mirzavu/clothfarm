...
class ModelSettingStore extends Model {
	public function addStore($data) {
		...
		if( $store_id && $this->db->escape($data['config_email']) ){
			$this->db->query("INSERT INTO " . DB_PREFIX . "merchant SET user_group_id = '1', store_ids = '" . (int)$store_id . "', username = '". $this->db->escape($data['config_name']) ."',  `firstname` = '" . $this->db->escape($data['firstname']) . "', `lastname` = '" . $this->db->escape($data['lastname']) . "', email = '". $this->db->escape($data['config_email']) ."', salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', date_added = NOW()");
	...
	public function editStore($store_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "merchant SET username = '" . $this->db->escape($data['config_name']) . "', `firstname` = '" . $this->db->escape($data['firstname']) . "', `lastname` = '" . $this->db->escape($data['lastname']) . "' WHERE store_ids = '" . (int)$store_id . "' AND user_id = '" . (int)$data['user_id'] . "' AND parent = 0 ");
