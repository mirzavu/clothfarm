...
class ModelDesignBanner extends Model {
	public function addBanner($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "banner SET name = '" . $this->db->escape($data['name']) . "', status = '" . (int)$data['status'] . "', vendor_id = '".$this->session->data['storeID']."'");
	...
	public function getBanner($banner_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "banner WHERE banner_id = '" . (int)$banner_id . "' AND vendor_id = '".$this->session->data['storeID']."'");

		return $query->row;
	}

	public function getBanners($data = array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "banner WHERE vendor_id = '".$this->session->data['storeID']."'";
	...
	public function getBannerImages($banner_id) {
		...
		$banner_image_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "banner_image bi LEFT JOIN " . DB_PREFIX . "banner b on b. banner_id = bi.banner_id WHERE bi.banner_id = '" . (int)$banner_id . "' AND b.vendor_id = '". $this->session->data['storeID'] ."' ORDER BY sort_order ASC");
	...
	public function getTotalBanners() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "banner WHERE vendor_id = '".$this->session->data['storeID']."'");
