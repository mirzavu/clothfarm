...
class ModelReportSale extends Model {
	...
	public function getTotalSales($data = array()) {
		$sql = "SELECT SUM(total) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id > '0' AND FIND_IN_SET('".$this->session->data['storeID']."', merchant_ids)>0";
	...
	public function getOrders($data = array()) { 
		$sql = "SELECT COUNT(DISTINCT o.order_id) AS orders, SUM(op.quantity) AS products, SUM(op.tax*op.quantity) AS tax, SUM(op.shipping) AS shipping, ( ( COALESCE(SUM(op.total),0)+COALESCE(SUM(cpv.value),0) ) ) AS total, ( ( COALESCE(SUM(op.tax*op.quantity),0)+COALESCE(SUM(op.shipping),0)+COALESCE(SUM(op.total),0)+COALESCE(SUM(cpv.value),0) ) ) AS main_total, MIN(o.date_added) AS date_start, MAX(o.date_added) AS date_end FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_product` op on o.order_id=op.order_id LEFT JOIN `" . DB_PREFIX . "order_status_vendor` osv on o.order_id=osv.order_id LEFT JOIN " . DB_PREFIX . "coupon_product_vendor cpv on o.order_id = cpv.order_id AND op.product_id = cpv.product_id ";
		...
		$sql .= " AND op.merchant_id='". $this->session->data['storeID'] ."' AND FIND_IN_SET('".$this->session->data['storeID']."', o.merchant_ids)>0 AND osv.vendor_id = '". $this->session->data['storeID'] ."'";
	...
	public function getTotalOrders($data = array()) {
		...
		$sql .= " LEFT JOIN `" . DB_PREFIX . "order_product` op on o.order_id=op.order_id LEFT JOIN `" . DB_PREFIX . "order_status_vendor` osv on o.order_id=osv.order_id LEFT JOIN " . DB_PREFIX . "coupon_product_vendor cpv on o.order_id = cpv.order_id AND op.product_id = cpv.product_id ";
		$sql .= " WHERE o.order_status_id > '0'";
		if (!empty($data['filter_order_status_id'])) {
			$sql .= " AND osv.order_status = '" . (int)$data['filter_order_status_id'] . "'";
		} 
	...
	public function getTaxes($data = array()) { 		
		...
		$sql = "SELECT MIN(o.date_added) AS date_start, MAX(o.date_added) AS date_end, COUNT(DISTINCT o.order_id) AS `orders`, otv.title, sum(otv.value) AS total FROM `" . DB_PREFIX . "order` o INNER JOIN `" . DB_PREFIX . "order_product` op ON (o.order_id = op.order_id) INNER JOIN `" . DB_PREFIX . "order_total_vendor` otv on op.product_id=otv.product_id and op.order_id= otv.order_id INNER JOIN `" . DB_PREFIX . "order_status_vendor` osv on o.order_id=osv.order_id WHERE otv.code = 'tax' AND op.merchant_id = '". $this->session->data['storeID'] ."' AND osv.vendor_id = '". $this->session->data['storeID'] ."' AND FIND_IN_SET('".$this->session->data['storeID']."', o.merchant_ids)>0 "; 
		...
		switch($group) {
			case 'day';
				$sql .= " GROUP BY YEAR(o.date_added), MONTH(o.date_added), DAY(o.date_added), otv.title";
				break;
			default:
			case 'week':
				$sql .= " GROUP BY YEAR(o.date_added), WEEK(o.date_added), otv.title";
				break;
			case 'month':
				$sql .= " GROUP BY YEAR(o.date_added), MONTH(o.date_added), otv.title";
				break;
			case 'year':
				$sql .= " GROUP BY YEAR(o.date_added), otv.title";
				break;
		}
	...
	public function getTotalTaxes($data = array()) {
		...
		switch($group) {
			case 'day';
				$sql = "SELECT COUNT(DISTINCT YEAR(o.date_added), MONTH(o.date_added), DAY(o.date_added), otv.title) AS total FROM `" . DB_PREFIX . "order` o";
				break;
			default:
			case 'week':
				$sql = "SELECT COUNT(DISTINCT YEAR(o.date_added), WEEK(o.date_added), otv.title) AS total FROM `" . DB_PREFIX . "order` o";
				break;
			case 'month':
				$sql = "SELECT COUNT(DISTINCT YEAR(o.date_added), MONTH(o.date_added), otv.title) AS total FROM `" . DB_PREFIX . "order` o";
				break;
			case 'year':
				$sql = "SELECT COUNT(DISTINCT YEAR(o.date_added), otv.title) AS total FROM `" . DB_PREFIX . "order` o";
				break;
		}
	...
	public function getShipping($data = array()) { 
		$sql = "SELECT COUNT(DISTINCT o.order_id) AS orders, SUM(op.shipping) AS total, MIN(o.date_added) AS date_start, MAX(o.date_added) AS date_end FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_product` op on o.order_id=op.order_id LEFT JOIN `" . DB_PREFIX . "order_status_vendor` osv on o.order_id=osv.order_id LEFT JOIN " . DB_PREFIX . "coupon_product_vendor cpv on o.order_id = cpv.order_id AND op.product_id = cpv.product_id ";
	...
	public function getTotalSalesByYear() {  
		...
		/*$query = $this->db->query("SELECT SUM(o.total) AS total, o.date_added, os.order_status_id FROM `" . DB_PREFIX . "order` o LEFT JOIN ".DB_PREFIX."order_status os on o.order_status_id = os.order_status_id WHERE o.order_status_id > 0 AND os.language_id = '" . (int)$this->config->get('config_language_id') . "' AND YEAR(o.date_added) = YEAR(NOW()) GROUP BY MONTH(o.date_added), o.order_status_id "); */

	$query = $this->db->query("SELECT SUM(o.total) AS total, o.date_added FROM `" . DB_PREFIX . "order` o WHERE o.order_status_id > 0 AND FIND_IN_SET('".$this->session->data['storeID']."', merchant_ids)>0 AND YEAR(o.date_added) = YEAR(NOW()) GROUP BY MONTH(o.date_added) ");
	...
	public function getTotalSalesByDay() {
		...
		$query = $this->db->query("SELECT SUM(total) AS total, HOUR(date_added) AS hour FROM `" . DB_PREFIX . "order` WHERE order_status_id > 0 AND FIND_IN_SET('".$this->session->data['storeID']."', merchant_ids)>0 AND DATE(date_added) = DATE(NOW()) GROUP BY HOUR(date_added) ORDER BY date_added ASC");
	...
	public function getTotalSalesByWeek() {
		...
		$query = $this->db->query("SELECT SUM(total) AS total, date_added FROM `" . DB_PREFIX . "order` WHERE order_status_id > 0 AND FIND_IN_SET('".$this->session->data['storeID']."', merchant_ids)>0 AND DATE(date_added) >= DATE('" . $this->db->escape(date('Y-m-d', $date_start)) . "') GROUP BY DAYNAME(date_added)");
	...
	public function getTotalSalesByMonth() {
		...
		$query = $this->db->query("SELECT SUM(total) AS total, date_added FROM `" . DB_PREFIX . "order` WHERE order_status_id > 0 AND FIND_IN_SET('".$this->session->data['storeID']."', merchant_ids)>0 AND DATE(date_added) >= '" . $this->db->escape(date('Y') . '-' . date('m') . '-1') . "' GROUP BY DATE(date_added)");
