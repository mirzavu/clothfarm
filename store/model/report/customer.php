...
class ModelReportCustomer extends Model {
	...
	public function getOrders($data = array()) {
		$sql = "SELECT c.customer_id, CONCAT(c.firstname, ' ', c.lastname) AS customer, c.email, cgd.name AS customer_group, c.status, COUNT(DISTINCT o.order_id) AS orders, SUM(op.quantity) AS products, SUM(op.tax) AS tax, SUM(op.total) AS `total` FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_product` op ON (o.order_id = op.order_id) LEFT JOIN `" . DB_PREFIX . "customer` c ON (o.customer_id = c.customer_id) LEFT JOIN `" . DB_PREFIX . "customer_group_description` cgd ON (c.customer_group_id = cgd.customer_group_id) LEFT JOIN " . DB_PREFIX . "customer_to_store cts ON (c.customer_id = cts.customer_id) LEFT JOIN `" . DB_PREFIX . "order_status_vendor` osv on o.order_id=osv.order_id WHERE cts.store_id != 0 AND cts.store_id = '".$this->session->data['storeID']."' AND op.merchant_id = '".$this->session->data['storeID']."' AND FIND_IN_SET('".$this->session->data['storeID']."', o.merchant_ids)>0 AND osv.vendor_id = '". $this->session->data['storeID'] ."' AND o.customer_id > 0 AND cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getTotalOrders($data = array()) {
		$sql = "SELECT COUNT(DISTINCT o.customer_id) AS total FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_product` op ON (o.order_id = op.order_id) LEFT JOIN `" . DB_PREFIX . "customer` c ON (o.customer_id = c.customer_id) LEFT JOIN `" . DB_PREFIX . "customer_group_description` cgd ON (c.customer_group_id = cgd.customer_group_id) LEFT JOIN " . DB_PREFIX . "customer_to_store cts ON (c.customer_id = cts.customer_id) LEFT JOIN `" . DB_PREFIX . "order_status_vendor` osv on o.order_id=osv.order_id WHERE cts.store_id != 0 AND cts.store_id = '".$this->session->data['storeID']."' AND op.merchant_id = '".$this->session->data['storeID']."' AND FIND_IN_SET('".$this->session->data['storeID']."', o.merchant_ids)>0 AND osv.vendor_id = '". $this->session->data['storeID'] ."' AND o.customer_id > 0 AND cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getCustomersOnline($data = array()) {
		$sql = "SELECT co.ip, co.customer_id, co.url, co.referer, co.date_added FROM " . DB_PREFIX . "customer_online co LEFT JOIN " . DB_PREFIX . "customer c ON (co.customer_id = c.customer_id) LEFT JOIN " . DB_PREFIX . "customer_to_store cts ON (c.customer_id = cts.customer_id) WHERE cts.store_id != 0 AND cts.store_id = '".$this->session->data['storeID']."'";
	...
	public function getTotalCustomersOnline($data = array()) {
		$sql = "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "customer_online` co LEFT JOIN " . DB_PREFIX . "customer c ON (co.customer_id = c.customer_id) LEFT JOIN " . DB_PREFIX . "customer_to_store cts ON (c.customer_id = cts.customer_id) WHERE cts.store_id != 0 AND cts.store_id = '".$this->session->data['storeID']."'";
