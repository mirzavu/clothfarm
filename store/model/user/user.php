...
class ModelUserUser extends Model {
	...
	public function getUser($user_id) {
		$query = $this->db->query("SELECT *, (SELECT ug.name FROM `" . DB_PREFIX . "user_group` ug WHERE ug.user_group_id = m.user_group_id) AS user_group FROM `" . DB_PREFIX . "merchant` m WHERE m.user_id = '" . (int)$user_id . "'");
	...
	public function getTotalUsersByEmail($email) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "merchant` WHERE LCASE(email) = '" . $this->db->escape(utf8_strtolower($email)) . "'");
