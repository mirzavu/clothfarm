...
class ControllerModuleDSocialLogin extends Controller {
	protected function index($setting) {
		...
		if(!isset($settings['base_url_index'])){ $settings['base_url_index'] = false; }

		if($settings['base_url_index']){
			$settings['base_url'] = $this->url->link('module/d_social_login/hybridauth', '', 'SSL');
		}else{
			$settings['base_url'] = $server.'catalog/model/d_social_login/hybridauth.php';
		}
		
		$settings['debug_file'] = DIR_SYSTEM . "logs/d_social_login.txt";
		...
		if(isset($this->request->get['provider'])){
			...
			require_once( DIR_APPLICATION."model/d_social_login/Hybrid/Auth.php" );
			 ...
			 try{
			  	 ...
			  	 $hybridauth = new Hybrid_Auth( $settings );
			  	...
			  	$this->redirect(urldecode($this->url->link('account/login', '')));
			  }
...
*/
	public function register($provider,$profile){
		...
		if($profile){
			...
			$data = array(
							    ...
							    'fax' => '',
							    'company' => '',
							    'customer_group_id' => (isset($settings['customer_group']))? $settings['customer_group'] : '1',
							    'company_id' => '',
							    'tax_id' => '',
			...
			if($customer){
				...
				$this->redirect(urldecode($this->session->data['d_social_login']['return_url']));
			}else{
			...
			//checking if email sent
				...
				if(!$popup){
					...
					if(!$customer){
						...
						$this->redirect($this->return_to_page($settings['return_url'], $this->session->data['d_social_login']['return_url']));
					}else{
						...
						$this->redirect($this->session->data['d_social_login']['return_url']);
					}
				}else{
						$this->language->load('module/d_social_login');
						$sort_order = array(); 
						...
						$this->data['background_img'] = $settings['background_img'];
						$this->data['background_color'] = $settings['providers'][ucfirst($provider)]['background_color'];
						...
						$this->data['text_address_2'] = $this->language->get('text_address_1');
						...
						$this->data['text_email_intro'] = $this->language->get('text_email_intro');
						
						$this->data['button_sign_in_mail'] = $this->language->get('button_sign_in_mail');

						if(
							isset( $_SERVER['HTTPS'] ) && ( $_SERVER['HTTPS'] == 'on' || $_SERVER['HTTPS'] == 1 )
						|| 	isset( $_SERVER['HTTP_X_FORWARDED_PROTO']) && $_SERVER['HTTP_X_FORWARDED_PROTO'] == 'https'
						){
							$this->data['http'] = HTTPS_SERVER;
						}
						else {
							$this->data['http'] = HTTP_SERVER;
						}

						if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/d_social_login/email.tpl')) {
							$this->template = $this->config->get('config_template') . '/template/d_social_login/email.tpl';
						} else {
							$this->template = 'default/template/d_social_login/email.tpl';
						}
	...
	public function create_with_email(){
		...
		if(!$error){
				...
				$this->provider_edit_customer($this->customer->getId(), $data);
				$json['url'] = $this->return_to_page($settings['return_url'], $this->session->data['d_social_login']['return_url']);
		}else{
			$json['error'] = $error; 
		}
