...
class ModelRewardPointsTransactions extends Model
{
    ...
    public function beforeUpdateOrder($order_id, $data)
    {
        ...
        if($data['order_status_id'] == $this->config->get('update_based_order_status'))
        {
            ...
            $this->db->query("UPDATE ".DB_PREFIX."customer_reward SET status = 1, customer_id = {$data['customer_id']} WHERE order_id = $order_id");
        }else{
            ...
            $this->db->query("UPDATE ".DB_PREFIX."customer_reward SET status = 0, customer_id = {$data['customer_id']} WHERE order_id = $order_id");
        }
    }
}