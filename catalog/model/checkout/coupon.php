...
class ModelCheckoutCoupon extends Model {
	...
	public function getOrderbyCoupon($order_id, $product_id) {
		$query = $this->db->query("SELECT otv.code,otv.title,otv.value,otv.sort_order FROM " . DB_PREFIX . "order_total_vendor otv WHERE otv.order_id = '". $order_id ."' AND otv.product_id = '".$product_id."' AND otv.code = 'coupon'"); 
