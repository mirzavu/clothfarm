...
class ModelCheckoutOrder extends Model { 
	public function addOrder($data, $order_status_id = "") { //print_r($data); exit;
		...
		foreach ($data['products'] as $product) {
			...
			$query = $this->db->query("SELECT *  FROM ". DB_PREFIX . "product_to_store WHERE product_id = '" . (int)$product['product_id'] . "' AND store_id != 0 ");
			$vendor = '';
			if(isset($query->row['store_id'])){				
				$storeIDS[] = $query->row['store_id'];	
				$vendor = $query->row['store_id'];
				$check = $this->db->query("SELECT *  FROM ". DB_PREFIX . "order_status_vendor WHERE order_id = '" . (int)$order_id . "' AND vendor_id = '" . $query->row['store_id'] . "'");

				if($check->num_rows == 0 ){
					$order_status_id == 0 ? $order_status_change = 1 : $order_status_change = $order_status_id;
					$this->db->query("INSERT INTO " . DB_PREFIX . "order_status_vendor SET order_id = '" . (int)$order_id . "', order_status = '".$order_status_change."', vendor_id = '" . $query->row['store_id'] . "', date_modified = NOW()");
				}

				$checkcustomer = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer_to_store WHERE customer_id = '" . (int)$data['customer_id'] . "' AND store_id = '" . $query->row['store_id'] . "'");
		...
		$unique_vendorIDS = array_unique($storeIDS);		
		$sIDS = implode(",",$unique_vendorIDS); 
		
		$this->db->query("UPDATE ". DB_PREFIX . "order SET merchant_ids = '". $sIDS ."' WHERE order_id = '" . $order_id . "' ");
		...
		if(count($data['taxes'])>0){
			foreach ($data['taxes'] as $taxes ) { 
				foreach ($taxes as $taxes_2 ) { 
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_total_vendor SET order_id = '" . (int)$order_id . "', product_id = '" . $this->db->escape($taxes_2['product_id']) . "', code = '" . $this->db->escape($taxes_2['code']) . "', tax_type = '".$this->db->escape($taxes_2['type'])."', title = '" . $this->db->escape($taxes_2['title']) . "', `value` = '" . (float)$taxes_2['value'] . "', sort_order = '". $this->db->escape($taxes_2['sort_order']) ."'");
				}
			}
		}	

		// coupons per product  ---- DONT DELETE 
	if (isset($data['totals'])) {
	foreach ($data['totals'] as $total_1) 
	{ 
		if( isset($total_1['code']) == 'coupon' && isset($total_1['couponProductID']) )
		{
			foreach($total_1['couponProductID'] as $key=>$cd)
			{
		$this->db->query("INSERT INTO " . DB_PREFIX . "order_total_vendor SET order_id = '" . (int)$order_id . "', product_id = '" .$key . "', code = '" . $this->db->escape($total_1['code']) . "', title = '" . $this->db->escape($total_1['title']) . "', `value` = '" . (float)$cd . "', sort_order = '". $this->db->escape($total_1['sort_order']) ."'");
		$this->db->query("INSERT INTO `" . DB_PREFIX . "coupon_product_vendor` SET coupon_id = '" . (int)$total_1['couponID'] . "', order_id = '" . (int)$order_id . "', product_id = '".$key."', `value` = '" . (float)$cd . "', customer_id = '" . (int)$data['customer_id']."', date_added = NOW() ");
		
		/*	if(isset($total_1['discount'])) {
				$this->db->query("UPDATE ". DB_PREFIX . "order_total_vendor SET value = (value - ((value * '".$total_1['discount']."') / 100)) WHERE order_id = '" . $order_id . "' AND product_id = '" .$key . "' AND tax_type = 'P'");
	...
	public function editOrder($order_id, $data, $vendor_id) {
		$this->event->trigger('pre.order.edit', $data);

		// Void the order first
		$this->addOrderHistory($order_id, 0, '', '', $vendor_id);
		...
		if($vendor_id){				
			$this->db->query("DELETE FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "' AND merchant_id = '". $vendor_id ."'");
		}else{
		...
		$order_product_id = $this->db->getLastId();
			...
			$check = $this->db->query("SELECT *  FROM ". DB_PREFIX . "order_status_vendor WHERE order_id = '" . (int)$order_id . "' AND vendor_id = '" . $vendor_id . "'");

			if($check->num_rows == 0 ){
			$this->db->query("INSERT INTO " . DB_PREFIX . "order_status_vendor SET order_id = '" . (int)$order_id . "', order_status = '1', vendor_id = '" . $vendor_id . "', date_modified = NOW()");
			}

			$this->db->query("DELETE FROM " . DB_PREFIX . "order_total_vendor WHERE order_id = '" . (int)$order_id . "' AND product_id = '". $product['product_id'] ."' AND code = 'tax' ");
	...
	public function deleteOrder($order_id, $vendor_id, $arranged_vendors) {
		...
		if($vendor_id == 0){		
			...
			$this->db->query("DELETE FROM `" . DB_PREFIX . "coupon_product_vendor` WHERE order_id = '" . (int)$order_id . "' ");

		}else{
			$this->db->query("UPDATE `" . DB_PREFIX . "order` SET merchant_ids='".$arranged_vendors."' WHERE order_id = '" . (int)$order_id . "'");	
		}
			
		$this->db->query("DELETE FROM `" . DB_PREFIX . "order_product` WHERE order_id = '" . (int)$order_id . "' AND merchant_id = '" . (int)$vendor_id . "'");
		
		$this->db->query("DELETE FROM `" . DB_PREFIX . "order_total_vendor` WHERE order_id = '" . (int)$order_id . "' AND vendor_id = '" . (int)$vendor_id . "'");

		
	
		
		$this->db->query("DELETE FROM `" . DB_PREFIX . "order_history` WHERE order_id = '" . (int)$order_id . "' AND vendor_id = '" . (int)$vendor_id . "'");
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'vendor_ids'              => $order_query->row['merchant_ids']
			);
		} else {
			return false;
		}
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $storesession = "", $flag = 1, $od_status = "") {
		...
		if($flag == 1){	 
			$od_status == 1 ? $order_status_idd = 1 : $order_status_idd = $order_status_id; 
			$this->db->query("UPDATE `" . DB_PREFIX . "order` SET order_status_id = '" . (int)$order_status_idd . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
		} 

$this->db->query("UPDATE `" . DB_PREFIX . "order_status_vendor` SET order_status = '" . (int)$order_status_id . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "' AND vendor_id = '" . (int)$storesession . "' ");

			$this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', order_status_id = '" . (int)$order_status_id . "', notify = '" . (int)$notify . "', comment = '" . $this->db->escape($comment) . "', date_added = NOW(), vendor_id = '" . (int)$storesession . "' ");
