...
$(document).ready(function(){
	...
	RewardPoints.Views.Behavior     =   Backbone.View.extend({
		...
		onChangeBehaviorType: function(ev){
            console.log(ev);
			var parent = $(ev.target).closest('#rule_actions_fieldset');
			if($(ev.target).val() == '2'){
				parent.find(".note").find("#post_review").show();
                parent.find('.consecutive-in-day').hide();
			}else if($(ev.target).val() == '9'){
                parent.find('.consecutive-in-day').show();
    ...
    view_action_on_behavior = new RewardPoints.Views.Behavior();
