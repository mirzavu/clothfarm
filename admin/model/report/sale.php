...
class ModelReportSale extends Model {
	...
	// Orders
	public function getTotalOrdersByDay($condition = "") {
		$implode = array();
	...
	public function getTotalOrdersByWeek($condition = "") {
		$implode = array();
	...
	public function getTotalOrdersByMonth($condition = "") {
		$implode = array();
	...
	public function getTotalOrdersByYear($condition = "") {
		$implode = array();
	...
	$query = $this->db->query("SELECT SUM(o.total) AS total, o.date_added FROM `" . DB_PREFIX . "order` o WHERE o.order_status_id > 0 AND YEAR(o.date_added) = YEAR(NOW()) GROUP BY MONTH(o.date_added) ");
	...
	public function getTotalSalesByWeek() {
		...
		$query = $this->db->query("SELECT SUM(total) AS total, date_added FROM `" . DB_PREFIX . "order` WHERE order_status_id > 0 AND DATE(date_added) >= DATE('" . $this->db->escape(date('Y-m-d', $date_start)) . "') GROUP BY DAYNAME(date_added)");
	...
	public function getTotalSalesByMonth() {
		...
		$query = $this->db->query("SELECT SUM(total) AS total, date_added FROM `" . DB_PREFIX . "order` WHERE order_status_id > 0 AND DATE(date_added) >= '" . $this->db->escape(date('Y') . '-' . date('m') . '-1') . "' GROUP BY DATE(date_added)");
