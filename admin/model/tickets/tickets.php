...
class ModelTicketsTickets extends Model {
	public function addStore($data) {
		...
		$merchant_detail = $this->db->query("SELECT * FROM ". DB_PREFIX . "merchant WHERE user_id = ".$data['vendor']);
	
		$this->db->query("INSERT INTO " . DB_PREFIX . "ticket SET `subject` = '" . $this->db->escape($data['subject']) . "', `message` = '" . $this->db->escape($data['message']) . "', `merchant_id` = '" . $this->db->escape($data['vendor']) . "', `sender_email` = '" . $merchant_detail->rows[0]['email'] . "', `ticket_time` = '" . time() . "', `status` = '" . $this->db->escape($data['status']) . "'");
	...
	public function addticketChild($ticket_id,$data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "ticket SET status = '" . $this->db->escape($data['status']) . "' WHERE ticket_id = '" . $ticket_id . "'");

		$this->db->query("INSERT INTO " . DB_PREFIX . "ticket SET `message` = '" . $this->db->escape($data['message']) . "', `ticket_time` = '" . time() . "', `sender_email` = '" . $store_data[0]['sender_email'] . "', `recipient` = '" . $store_data[0]['recipient'] . "', `status` = '" . $this->db->escape($data['status']) . "', `parent_id` = '" . $ticket_id . "', `merchant_id` = '" . $store_data[0]['merchant_id'] . "'");
	...
	public function getTickets($data = array()) {
		...
		if (!$store_data) {
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "ticket  WHERE parent_id = 0 ORDER BY ticket_id DESC");
	...
	public function getTotalTickets() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "ticket WHERE parent_id = 0");
