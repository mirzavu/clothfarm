...
class ControllerFundsVendorPayment extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('funds/vendor_payment', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$pagination->url = $this->url->link('funds/vendor_payment', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
	...
	public function debits() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('funds/vendor_payment/debits', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);

		$data['add'] = $this->url->link('funds/vendor_payment/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('funds/vendor_payment/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');		
		$data['debits'] = array();
		...
		foreach ($results as $result) {
			...
			$data['debits'][] = array(
				...
				'delete'      => $this->url->link('funds/vendor_payment/delete', 'token=' . $this->session->data['token'] . '&debit_id=' . $result['refund_id'] . $url, 'SSL')
			);
		}
		...
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('funds/vendor_payment/debits', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('funds/vendor_payment/debits', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function getForm() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('funds/vendor_payment/add', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);

		if (!isset($this->request->get['refund_id'])) {
			$data['action'] = $this->url->link('funds/vendor_payment/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('funds/vendor_payment/edit', 'token=' . $this->session->data['token'] . '&review_id=' . $this->request->get['refund_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('funds/vendor_payment/debits', 'token=' . $this->session->data['token'] . $url, 'SSL');
	...
	protected function validateForm() { 
		if (!$this->user->hasPermission('modify', 'funds/vendor_payment')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}

		
		if ($this->request->post['debit'] || $this->request->post['dispute']) { 
			if ($this->request->post['debit'] == 0 && $this->request->post['dispute'] == 0) {
				$this->error['warning'] = $this->language->get('error_debit');
			}
		}else if(!is_numeric($this->request->post['debit']) || !is_numeric($this->request->post['dispute'])){
			$this->error['warning'] = $this->language->get('error_debit');			
		}
		else{
			$this->error['warning'] = $this->language->get('error_debit');
