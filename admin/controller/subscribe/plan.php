...
class ControllerSubscribePlan extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$store_id = $this->model_subscribe_plan->addPlan($this->request->post);
			...
			$this->response->redirect($this->url->link('subscribe/plan', 'token=' . $this->session->data['token'], 'SSL'));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->model_subscribe_plan->editPlan($this->request->get['plan_id'], $this->request->post);
			...
			$this->response->redirect($this->url->link('subscribe/plan', 'token=' . $this->session->data['token'] . '&plan_id=' . $this->request->get['plan_id'], 'SSL'));
		}
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $plan_id) {
				$this->model_subscribe_plan->deletePlan($plan_id);
			}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('subscribe/plan', 'token=' . $this->session->data['token'], 'SSL')
		);

		
		$data['add'] = $this->url->link('subscribe/plan/add', 'token=' . $this->session->data['token'], 'SSL');
		$data['delete'] = $this->url->link('subscribe/plan/delete', 'token=' . $this->session->data['token'], 'SSL');

		$store_total = $this->model_subscribe_plan->getTotalplans();
	
	
		$results = $this->model_subscribe_plan->getPlans();
		...
		foreach ($results as $result) {
			$data['plans'][] = array(
				...
				'edit'     => $this->url->link('subscribe/plan/edit', 'token=' . $this->session->data['token'] . '&plan_id=' . $result['plan_id'], 'SSL')
			);
		}
	...
	public function getForm() {
		...
		if (!isset($this->request->get['plan_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('subscribe/plan/add', 'token=' . $this->session->data['token'], 'SSL')
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('text_settings'),
				'href' => $this->url->link('subscribe/plan/edit', 'token=' . $this->session->data['token'] . '&plan_id=' . $this->request->get['plan_id'], 'SSL')
			);
		}
		...
		if (!isset($this->request->get['plan_id'])) {
			$data['action'] = $this->url->link('subscribe/plan/add', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('subscribe/plan/edit', 'token=' . $this->session->data['token'] . '&plan_id=' . $this->request->get['plan_id'], 'SSL');
		}
			
		$data['cancel'] = $this->url->link('subscribe/plan', 'token=' . $this->session->data['token'], 'SSL');
		
		if (isset($this->request->get['plan_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
		
			$this->load->model('subscribe/plan');
			
			$store_info = $this->model_subscribe_plan->getSetting($this->request->get['plan_id']);
		...
		if (isset($this->request->post['highlight'])) {
			$data['highlight'] = 1;
		} elseif (isset($store_info->row['highlight'])) {
			$data['highlight'] = $store_info->row['highlight'];
		} else {
			$data['highlight'] = 0;
		}
		...
		} elseif (isset($store_info->row['description'])) {
			$data['description'] = $store_info->row['description'];
		} else {
			$data['description'] = '';
		}
		...
		$this->response->setOutput($this->load->view('subscribe/plan_form.tpl', $data));
	}
	...
	public function vendor_list()
	{
		...
		$results = $this->model_subscribe_plan->getSubscriberplanlist();
		
	
		foreach ($results as $result) {
		
			$merchant_email = $this->model_subscribe_plan->getcurrentemail($result['merchant_id']);
			$data['plans'][] = array(
		...
		$this->response->setOutput($this->load->view('subscribe/plan_vendor_list.tpl', $data));
	}
}
